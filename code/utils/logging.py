...
class LogConfParser(BaseConfParser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        log_args.add_argument(
            '-lg.f', '--log.file', type=str, metavar='FILE',
            help="Path to the logging file")
        log_args.add_argument(
            '-lg.l', '--log.level', default='info', type=str,
...
def init_logging(cfg: Munch):
    ...
    """
    file = cfg.log.file if 'file' in cfg.log else None
    level = cfg.log.level
...
class DataLogger:
    ...
    def __init__(self, cfg: Munch, tag_suffix: Optional[str] = None) -> None:
        if cfg.log.tensorboard_log_dir is not None:
        ...
        ) if cfg.log.tensorboard_log_dir is not None else None)

        if cfg.log.data_file is not None:
            pathlib.Path(os.path.dirname(cfg.log.data_file)).mkdir(
                parents=True, exist_ok=True)

        self.csv_file = (open(
            cfg.log.data_file, 'a', encoding='utf-8', newline=''
        ) if cfg.log.data_file is not None else None)
