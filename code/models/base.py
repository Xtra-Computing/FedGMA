...
class _ModelTrainer(ABC):
    ...
    def __init__(
        ...
        if self.use_dp and cfg.differential_privacy.dp_type == 'rdp':
            computed_delta = (
                min(1e-5, 1. / len(data_loader))
                if cfg.differential_privacy.rdp.delta == 0
                else cfg.differential_privacy.rdp.delta)
            self.noise_level = compute_gaussian_sigma(
                cfg.differential_privacy.rdp.epsilon, computed_delta,
                cfg.training.batch_size, len(data_loader),
                cfg.training.epochs)
